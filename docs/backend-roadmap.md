...
> 状态：M1–M5 分期与两个关键分叉已拍板。本文是后端化的唯一路线参照。
> 依据：前端原型（`frontend/src/shared/agents.tsx` + `agent-types.ts`/`agent-seeds.ts`）已定义全部数据模型；后端现状为单会话内核 + SQLite（sessions/messages/projects）。

## 拍板记录

| 分叉 | 决策 | 时间 |
|---|---|---|
| 子 Agent 执行模型 | **子上下文隔离**——dispatch 开独立执行上下文（任务描述 + 子 Agent 四层组合），跑完只把结果回填主会话；主上下文不膨胀、子 Agent 看不到无关背景（Claude Code 同款） | 2026-09-18 |
...
- 多活跃会话/调度器/worktree（AGENTS.md §2.1 未来规划——单会话 + 顺序 dispatch 足够）
- 子 Agent 再委派（两类制结构性禁止——深度恒 1）
- 子 Agent 的持久化会话树（子上下文随主会话轮次结束丢弃；将来要看子过程再加持久化——事件流已经带 dispatchId 可回放）
